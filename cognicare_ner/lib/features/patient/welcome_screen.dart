...
class _WelcomeScreenState extends State<WelcomeScreen> {
  ...
  Future<void> _initMedia() async {
    ...
    if (welcomeVideo != null) {
      _videoController = mediaVideoController(
        url: welcomeVideo.url,
        path: welcomeVideo.localPath,
      );
    ...
    if (welcomeAudio != null) {
      try {
        ...
        } else if (isLocalFilePath(welcomeAudio.localPath)) {
          await _audioPlayer.setFilePath(welcomeAudio.localPath!);
        } else if (welcomeAudio.localPath != null && welcomeAudio.localPath!.isNotEmpty) {
          await _audioPlayer.setUrl(welcomeAudio.localPath!);
        }
                              ...
                              ? TweenAnimationBuilder<double>(
                                  ...
                                  child: Container(
                                    ...
                                    child: ClipRRect(
                                      borderRadius: BorderRadius.circular(24),
                                      child: MediaImage(
                                        src: (welcomePhoto.localPath?.isNotEmpty ?? false)
                                            ? welcomePhoto.localPath
                                            : welcomePhoto.url,
